...
class Kama_Contents implements Kama_Contents_Interface
{
    ...
    protected function collect_toc(string &$content, array $tags): void
    {
        ...
        // // Перевірка на відкриті теги та додавання закриваючих тегів
        if (preg_match_all('/<div[^>]*>/', $new_content, $matches)) {
            // Перевірка, чи є відкриті <div> без закритих
            $open_div_count = count($matches[0]);

            // Якщо є відкриті <div>, додаємо закриваючі
            if ($open_div_count > substr_count($new_content, '</div>')) {
                $content .= str_repeat('</div>', $open_div_count - substr_count($new_content, '</div>'));
            }
        }
    }
    ...
    /**
     * Callback function to wrap headings and content in nested <div>.
     */
    protected function collect_toc_replace_callback(array $match): string
    {
        ...
        return $this->_wrap_in_div($elem->level, $new_heading, $elem->anchor);
    }

    /**
     * Wrap content in nested <div> based on heading levels, using the same ID as the heading.
     */
    protected function _wrap_in_div(int $level, string $content, string $id): string
    {
        static $last_level = 0;
        static $open_divs = [];
        ...
        while ($last_level >= $level && !empty($open_divs)) {
            $html .= '</div>';
            array_pop($open_divs);
            $last_level--;
        }
...
trait Kama_Contents__Html
{
    ...
    protected function _toc_html(): string
    {
        ...
        foreach ($this->toc_elems as $elem) {
            ...
            if ($elem->level === 0) {
                ...
                $parent_h2 = "<div class='fs-toc_link-wrapper-2 is-h2'>{$elem_html}";
            } else {
    ...
    protected function toc_html(): string
    {
        ...
        // list
            ...
            if ($add_wrapper) {
                ...
                $contents_wrap_patt = '
					<div fs-toc-element="table" {ItemList}>
                    <div class="padding-bottom padding-small">
                    <div class="text-font-sora text-size-large text-weight-semibold text-color-alternate">' . $this->opt->title . '</div>
                    </div>
    ...
    protected function render_item_html(TOC_Elem $elem): string
    {
        ...
        // list (li)
            ...
            if ($elem->level === 0) {
                $elem_html = '
                    <a href="' . "{$this->opt->page_url}#$elem->anchor" . '" class="fs-toc_link-4 w-inline-block"><div fs-toc-element="link">' . $elem->text . '</div></a>
                ...
                $elem_html = '
                    <div class="fs-toc_link-wrapper-2 is-h3" style="height: 0px;">
                        <a href="' . "{$this->opt->page_url}#$elem->anchor" . '" class="fs-toc_link-4 is-h3 w-inline-block"><div fs-toc-element="link">' . $elem->text . '</div></a>
                    </div>';
            }
        }
